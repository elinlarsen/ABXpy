...
class Task(object):
    ...
    def on_across_triplets(self, by, on, across,
                           ...
                           with_regressors=True, max_samples=None):
        ...
        max_samples : numpy.int64
            Max triplets by on-across-by cell
        ...
        if not with_regressors:
            ...
            regressors = self._compute_regressors(
                triplets, iA, iB, iX,
                ABX_filter_ind, ABX_sample_ind, thr_sort_permut)
    ...
    def generate_triplets(self, output=None, max_samples=None,
        ...
        max_samples : int, optional
            Maximum number of triplets per on-across-by cell. If a
            cell have more triplets than that maximum, the cell is
            capped to that maximum value (`max_samples` triplets are
            randomly selected in the cell).
        ...
        if max_samples is None:
            ...
            # check no triplets approximation
            if self.stats['approximate_nb_triplets']:
                raise ValueError(
                    'Cannot sample if number of triplets '
                    'is computed approximately')

            # issue a warning if max_sample is a float (float value
            # was allowed with previous implementations of sampling)
            if isinstance(max_samples, float):
                warnings.warn(
                    'max_sample is a float, please use an integer',
                    UserWarning)
            ...
            for by, db in self.by_dbs.iteritems():
                ...
                with h5io.H5IO(
                    ...
                    self._compute_triplets(
                        ...
                        by_values, max_samples, display=display)
    ...
    def _compute_triplets(self, by, out, out_block_index,
                          out_regs, db, fh, by_values,
                          max_samples, display=None):
...
Example call:
  ...
  are equivalent:
    ...
    g2.add_argument(
        '-K', '--max-samples', default=None, type=np.int64,
        help='maximum number of triplets in each on-across-by cell '
        '(no max by default)')
